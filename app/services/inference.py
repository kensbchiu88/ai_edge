...
class InferenceService:
    ...
    def start_pipeline(self, session, input: 'InferenceService.ClassifyStreamingInputDto'):
        if input.uri:
            if input.uri in self.running_threads: 
                ...
                task_thread = threading.Thread(target=self.__classify_streaming_task, args=(event, session, input))
    ...
    def __classify_streaming_task(self, stop_event, session, input: 'InferenceService.ClassifyStreamingInputDto'):
        result_list = []
        mqtt_client = None
        ...
        streaming_repository = StreamingRepository(session)
        streaming_data_id = streaming_repository.get_streaming_by_uri(input.uri).id
        label_repository = LabelRepository(session)
        label_data = label_repository.get_labels_by_streaming_id(streaming_data_id)
    ...
    def __get_mqtt_client(self, mqtt_host, mqtt_topic):
        if mqtt_host and mqtt_host.strip():
            raise AppError(message="mqtt topic is not empty.")
